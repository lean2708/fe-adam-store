...
export function ProductVariantsStats({
  variants,
  totalElements,
}: ProductVariantsStatsProps) {
  const t = useTranslations('Admin.products');
  const locale = useLocale();

  const activeProducts = variants.filter((p) => p.status === 'ACTIVE').length;
  const inactiveProducts = variants.filter(
    (p) => p.status === 'INACTIVE'
  ).length;
  ...
  const totalVariants = variants.reduce((sum, product) => {
    return (
      ...
      (product.colors?.reduce(
        (colorSum, color) => colorSum + (color.variants?.length || 0),
        0
      ) || 0)
    );
  }, 0);
  ...
  const allVariants = variants.flatMap(
    (product) => product.colors?.flatMap((color) => color.variants || []) || []
  );
  const averagePrice =
    allVariants.length > 0
      ? allVariants.reduce((sum, variant) => sum + (variant.price || 0), 0) /
        allVariants.length
      : 0;
  ...
  const stats = [
    {
      ...
      description: `${activeProducts} ${
        t('activeProducts') || 'hoạt động'
      }, ${inactiveProducts} ${t('inactiveProducts') || 'không hoạt động'}`,
      color: 'text-blue-600',
      bgColor: 'bg-blue-50',
    },
    {
    ...
    },
    {
      ...
      description: `${availableProducts} ${
        t('availableProducts') || 'sản phẩm có sẵn'
      }`,
    ...
    <div className='grid gap-2 md:gap-4 grid-cols-2 lg:grid-cols-4'>
      {stats.map((stat, index) => {
        ...
        return (
          <Card
            key={index}
            className='bg-white dark:bg-gray-900 md:border border-border rounded-lg md:shadow-sm'
          >
            <CardHeader className='flex flex-row  p-4 md:p-6 items-center justify-between space-y-0 pb-2'>
