...
export function ChatInput({
  ...
  const handleSendMessage = () => {
    if (!message.trim() || !conversation?.id || disabled) return;
  ...
  const isDisabled = disabled || !conversation?.id || !isWebSocketConnected;
    ...
    <div className="p-4 border-t bg-white">
      <div className="flex items-center space-x-2">
        ...
        <div className="flex-1 relative">
          <Input
            placeholder={
              !conversation 
                ? (t("selectConversationFirst") || "Chọn cuộc trò chuyện để bắt đầu...")
      ...
      {!conversation && isWebSocketConnected && (
        <p className="text-xs text-gray-500 mt-2">
          {t("selectConversationToStart") || "Chọn cuộc trò chuyện để bắt đầu nhắn tin"}
        </p>
      )}
        ...
        <div className="flex items-center justify-between mt-2">
          <p className="text-xs text-gray-500">
            {t("chatWith") || "Đang chat với"} {
              ...
              t("unknownCustomer") || "Khách hàng không rõ"
            }
          </p>
