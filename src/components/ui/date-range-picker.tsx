                ...
                <PopoverTrigger asChild>
                    <Button size={'lg'} variant="outline">
                        <div className="text-right">
                            <div className="py-1">
                                <div>{`${formatDate(range.from.toISOString(), locale, { year: 'numeric', month: 'short', day: 'numeric' })}${range.to != null ? ' - ' + formatDate(range.to.toISOString(), locale, { year: 'numeric', month: 'short', day: 'numeric' }) : ''
                                    }`}</div>
                            </div>
                            {rangeCompare != null && (
                                <div className="opacity-60 text-xs -mt-1">
                                    <>
                                        vs. {formatDate(rangeCompare.from.toISOString(), locale, { year: 'numeric', month: 'short', day: 'numeric' })}
                                        {rangeCompare.to != null
                                            ? ` - ${formatDate(rangeCompare.to.toISOString(), locale, { year: 'numeric', month: 'short', day: 'numeric' })}`
                                            : ''}
                                    </>
                                </div>
                            )}
