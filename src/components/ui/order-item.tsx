...
export default function OrderItem(props: { onDeleted: (id: number)=>void, id:number, activeStatus: TabStatus, totalPrice?: number, items?: TOrderItem[], openModule: () => void }) {
  const btnByStatus: Record<TabStatus, React.ReactNode> = {
    PENDING: <>
      ...
      <button onClick={() => openModule()} className='w-52 px-4 mr-4 py-2 rounded-md border border-[#888888] text-sm'>Thay đổi địa chỉ nhận hàng</button>
      <button onClick={() => { setIsDeleted(true) }} className='w-40 px-4 py-2 border-[#888888] border text-black rounded-md text-sm'>Huỷ đơn hàng</button>
    </>,
    PROCESSING: <>
      <button className="w-40 px-4 py-2 mr-4 rounded-md border border-[#C5C4C2] text-sm text-[#C5C4C2] bg-[#E5E4E1]">Chờ</button>
      <button onClick={() => openModule()} className='w-52 px-4 mr-4 py-2 rounded-md border border-[#888888] text-sm'>Thay đổi địa chỉ nhận hàng</button>
    ...
    SHIPPED: <>
      <button className='w-56 text-gray-400 px-4 py-2 rounded-md border border-gray-400 text-sm'>Xác nhận đã nhận hàng</button>
    </>,
    DELIVERED: <>
      <button className="w-40 px-4 py-2 border border-[#888888] rounded-md text-sm mr-4">Liện hệ chúng tôi</button>
      <button className='w-28 px-4 py-2 bg-black text-white rounded-md text-sm'>Mua lại</button>
    </>,
  ...
  return (
    <>
      ...
      <div className="w-full text-end pt-2">Tổng số tiền ({items.length} sản phẩm) &nbsp;&nbsp;&nbsp;<span className="text-xl font-bold">{totalPrice && formatCurrency(totalPrice)}</span></div>
      <div className='py-2 text-end'>
        {btnByStatus[activeStatus]}
      </div>
      <ConfirmDialogModule loading={loading} onClose={() => setIsDeleted(false)} title="Bạn có chắc muốn huỷ đơn hàng ?" onSubmit={() => { CloseOrder() }} confirm={isDeleted} />
    </>
...
function ItemProductOrder(props: { item: TOrderItem, active: TabStatus }) {
  ...
  return (
    <div className="border-b-1 border-dashed py-2 w-full flex justify-between min-h-25 items-center">
      <div className="flex ">
        <img className="h-25 rounded-sm" src={item.image?.imageUrl} alt={'' + item.image?.id} />
        <div className="h-full flex flex-col justify-between ml-3">
          <h4 className="font-bold">{item.productVariant?.product?.name}</h4>
          <p className="text-[#888888]">Màu sắc: {item.productVariant?.color?.name}</p>
          <p className="text-[#888888]">Kích cỡ: {item.productVariant?.size?.name}</p>
