...
import { createReviewAction, getReviewAction } from "@/actions/reviewAction";
...
export default function ReviewModule(props: { visible: boolean, orderItem?: TOrderItem, onClose: () => void }) {
  ...
  useEffect(() => {
    const getReviewById = async () => {
      try {
        if (orderItem?.id) {
          const res = await getReviewAction(orderItem.id);
  ...
  const uploadImg = async (file: any) => {
    return await uploadFileAction(file)
  }
  const CloseMoule = () => {
    setRating(0)
    setComment('')
      ...
      <Card className="relative w-full max-w-3xl bg-white dark:bg-gray-700 !p-6 rounded-lg shadow-lg" onClick={stopPropagation}>
        ...
        <div className="pt-4">
          ...
          <div className="border border-black dark:border-white p-3 rounded-lg flex">
            <img className="w-24 h-24" src={orderItem?.Product?.images[0] ?? imgFallback} alt={orderItem?.Product?.title} />
            <div className="ml-7 h-24 justify-between flex flex-col">
              <p className="text-xl font-bold">{orderItem?.Product?.title ?? 'Slim-Fit Stretch-Cotton Poplin Fabric Overshirt'}</p>
