 ...
 */
export async function transformCartItemResponseToTCartItem(
  apiCartItem: CartItemResponse
): Promise<TCartItem> {
 ...
 */
export async function transformCartItemResponseToTCartItemWithProduct(
  apiCartItem: CartItemResponse,
  productOverride?: TProduct
): Promise<TCartItem> {
  const variant = apiCartItem.productVariantBasic;
  ...
  const product: TProduct | null =
    productOverride ??
    (variant?.product?.id
      ? await fetchProductDetailByIdApi(variant.product.id)
      : null);
