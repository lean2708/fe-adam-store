 ...
 */
async function transformProductVariantBasicToProduct(
  orderItemResponse: OrderItemResponse
): Promise<TOrderItem['Product']> {
  return {
    id: orderItemResponse?.productVariant?.product?.id?.toString() ?? '',
    title:
      orderItemResponse?.productVariant?.product?.name ?? 'Unknown Product',
 ...
 */
export async function transformOrderItemResponseToTOrderItem(
  apiOrderItem: OrderItemResponse
): Promise<TOrderItem> {
  const productVariant = apiOrderItem.productVariant;
  const productData = productVariant
    ? await transformProductVariantBasicToProduct(apiOrderItem)
    : null;
  return {
 ...
 */
function transformApiStatusToOrderStatus(
  apiStatus?: OrderResponse['orderStatus']
): ORDER_STATUS {
 ...
 */
export async function transformOrderResponseToTOrder(
  apiOrder: OrderResponse
): Promise<TOrder> {
  const orderItemsPromises = (apiOrder.orderItems ?? []).map(
    transformOrderItemResponseToTOrderItem
  );
 ...
 */
export async function transformOrderArrayToTOrderArray(
...
export async function transformPageResponseOrderToActionResponse(
  ...
  try {
    const transformedOrders = await transformOrderArrayToTOrderArray(
      apiResponse.items ?? []
    );
    ...
    return {
      success: false,
      message:
        error instanceof Error ? error.message : 'Failed to transform orders',
    };
  }
}
